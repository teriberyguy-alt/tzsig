...
def scrape_ladder_stats(player_id):
        ...
        if table:
            ...
            for row in rows:
                ...
                if len(cells) >= 5:
                    ...
                    if "GuyT#11983" in name or "Its_Guy" in name:
                        stats['rank'] = rank
                        stats['level'] = cells[2].text.strip()
                        stats['class'] = cells[3].text.strip()
                        stats['exp'] = cells[4].text.strip()
                        stats['battletag'] = name
                        break

        # Last active
        last_active = soup.find(string=lambda t: "Last Active" in t if t else False)
        if last_active:
            stats['last_active'] = last_active.find_parent().get_text(strip=True).replace("Last Active:", "").strip()
